...
const Navbar = () => {
  ...
  const homeNavbar = 'absolute md:flex md:items-center md:justify-between w-full bg-opacity-75';
  const navbar = 'md:flex md:items-center md:justify-between w-full bg-white border-b-2';
  const homeLink = 'hover:bg-blue-dark hover:bg-opacity-75 hover:text-white rounded transition duration-150 px-2 py focus:outline-none focus:ring-2 focus:ring-blush-light focus:ring-opacity-50 uppercase tracking-wider text-gray-700 text-2xl';
  const navLink = 'hover:bg-lavender-dark hover:text-white rounded transition duration-150 px-2 py-2 focus:outline-none focus:ring-2 focus:ring-lavender-dark focus:ring-opacity-50 uppercase tracking-wider text-gray-700 text-2xl';

  const linkToggle = () => setIsOpen(false);
  const toggle = () => setIsOpen(!isOpen);

  const navbarMapper = () => Object.keys(NavbarLinks).map((link) => (
    <li key={ link.id } className="block px-2 py-2">
      <NavLink
